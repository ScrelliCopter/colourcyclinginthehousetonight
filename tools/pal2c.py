...
def main():
	...
	if len(sys.argv) == 3:
		...
		sys.exit(f"Usage: {sys.argv[0]} <image.LBM> [out.txt]")

	inPath = Path(sys.argv[1])
	if outPath is not None and inPath.resolve() == outPath.resolve():
		sys.exit("Woah there! Tried to write to the input file, stopping before things get spicy")
	...
	try:
		...
		sys.exit(f"Failed to open \"{e.filename}\": {e.strerror}")
