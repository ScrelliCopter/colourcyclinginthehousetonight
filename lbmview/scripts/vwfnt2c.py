...
def writekern(file: TextIO, spec: Spec, label: str = "GetKern"):
	...
	def explode(v: str, p: [str]) -> str:
		if len(p) <= 16 or next((c for c in p if 0 > ord(c) >= 0x80), None) is not None:
			return " || ".join(f"{v} == {cchar(c)}" for c in p)
		else:
			return f"strchr(\"{''.join(cchar(c, string=True) for c in p)}\", {v})"
